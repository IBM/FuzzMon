...
async function main() {
	try {
		...
		const BasicShellInjectionExpert = require(path.join(JSFuzzSrcDir, 'fuzzer/expert')).BasicShellInjectionExpert;
		...
		const BASE_DIR = path.join(JSFuzzDir, 'test', 'test_apps', 'shell_injection_example');
		const filesList = [path.join(BASE_DIR, 'shell_injection_example.js')];

		const entryFileName = path.join(BASE_DIR, 'shell_injection_example.js');
		const targetFileName = path.join(BASE_DIR, 'shell_injection_example.js');

		const targetsList = [new ShellInjectionTarget(targetFileName)];

		const expertType = BasicShellInjectionExpert;
		let inputs = ['asdasdfg'];
